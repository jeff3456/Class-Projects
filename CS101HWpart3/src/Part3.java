	...
	public void setup()
	{
		...
		try
		{
			...
			{				
				...
				int romney = Integer.parseInt(splitLine[1]), obama = Integer.parseInt(splitLine[2]);
				
				// if Romney has more votes then go through list of states and find the same identifier and change
				// color to blue
				if(romney > obama)
				{
					for(int i = 0; i < states.size(); i++)
					{
						if((splitLine[0].charAt(0) == states.get(i).name.charAt(0)) && (splitLine[0].charAt(1) == states.get(i).name.charAt(1)))
						{
							println("republican");
							states.get(i).blue = 0;
							states.get(i).red = 255;
						}
					}
				}
				// if Romney didn't win, then Obama probably has more votes, so change the color of shape to red
				else
				{
					for(int i = 0; i < states.size(); i++)
					{
